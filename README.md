...
- [错词复习](#错词复习)：章节结算后只重练本章拼错的单词，且不计入章节记录，不影响练习统计
...
默认开启，练习时隐藏单词的全部字母，由用户凭记忆拼写。可以在设置中改为隐藏元音、隐藏辅音或随机隐藏，也可以完全关闭，快捷键 `Ctrl + V` 随时切换。
...
章节结束的结算弹窗里会列出本章拼错的单词，点「复习错词」（快捷键 `r`）即可只重练这些词。练完仍有拼错的可以接着往下收敛，直到全部拿下；点「重复本章节」随时回到完整章节。

这一轮只练本章的一小撮单词，因此不写入章节练习记录，不会影响词库页的练习次数与平均错误数，也不会污染结果页的个人最好成绩。每个单词的练习记录照常保存，所以练对了就会自动移出错题本。
...
音频文件只在本机浏览器里解码、播放与存储，不会上传到任何服务器。

**复读机**

- 载入本地音频（mp3 / m4a / wav），点击「选择音频文件」或直接把文件拖到波形区
- 最近一次载入的音频会自动存在本机浏览器（IndexedDB）里，刷新或重新打开页面时自动恢复，无需再选一次；后一次载入会覆盖前一次
...
- 「精简界面」把走带按钮以下的控制区整段收起，只留文件名、计时器与波形条，正式听写时视线不受干扰；此时快捷键照常可用，开关状态会被记住

**听写本**

- 右侧横格纸风格的听写区，支持衬线 / 无衬线 / 等宽三种字体，实时统计词数与字符数
- 「插入时间码」在光标处插入当前播放位置，方便回头核对
- 内容自动保存在本机浏览器（localStorage）；需要留存或换设备时，直接在听写区里全选复制即可
- 「载入原文」导入该音频对应的文字稿（纯文本），点「对答案」才把原文展开，写的过程中不会剧透；原文按音频文件名配对存在本机，换回同一个音频时自动带出
- 原文按原样展示（保留换行），可以选中复制，比对交给你自己判断——机器判分难免在标点、连读、同义替换上给出误导性的结论
